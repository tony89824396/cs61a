...
def pingpong(n):
    ...
    """
    "*** YOUR CODE HERE ***"
    def recursive(location,number,add):
        if location==n:
            return number
        elif location %7==0 or has_seven(location):
            return recursive(location+1, number-add, add*(-1))
        else:
            return recursive(location+1, number+add, add)
